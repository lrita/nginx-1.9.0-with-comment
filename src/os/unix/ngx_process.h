...
typedef struct {
    ...
    ngx_socket_t        channel[2];	//这是由socketpair系统调用产生出的用于进程间通信的socket句柄，这一对socket句柄可
    					//以互相通信，目前用于master 父进程与worker子进程间的通信。

    ngx_spawn_proc_pt   proc;		//子进程的循环执行方法，当父进程调用ngx_spawn_process 生成子进程时使用
    void               *data;		//上面的ngx_spawn_proc_pt方法中第二个参数需要传递一个指针，它是可选的。例如worker
    					//子进程就不需要，而cache manage进程就需要ngx_cache_manager_ctx上下文成员。这时
    					//data一般与ngx_spawn_proc_pt方法中第二个参数是等价的。
