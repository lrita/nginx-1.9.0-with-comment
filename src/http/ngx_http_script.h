...
typedef struct {
    ...
    //下面二个变量和dup_capture放在一起讨论，他们都跟pcre的正则处理相关，这三个用到的地方比较少
    ngx_uint_t                  ncaptures;		//当前处理时，出现的$n变量的最大值，如配置的最大为$3，那么ncaptures就等于3
    ngx_uint_t                  captures_mask;		//以位移的形式保存$1,$2...$9等变量，即响应位置上置1来表示，主要的作用是
    							//为dup_capture准备，正是由于这个mask的存在，才比较容易得到是否有重复
    							//的$n出现。

    ngx_uint_t                  size;			//待compile的字符串中，"常量字符串"的总长度

    void                       *main;			//对于main这个成员，有许多要挖掘的东西。main一般用来指向一个
    ...
    unsigned                    dup_capture:1;		//这个标记位主要在rewrite模块里使用，在ngx_http_rewrite中，
    							//if (sc.variables == 0 && !sc.dup_capture) {
    							//	regex->lengths = NULL;
    							//} 
							//没有重复的$n，那么regex->lengths被置为NULL，这个设置很关键，在函数
							//ngx_http_script_regex_start_code中就是通过对regex->lengths的判断，
							//来做不同的处理，因为在没有重复的$n的时候，可以通过正则自身的captures
    							//机制来获取$n，一旦出现重复的，那么pcre正则自身的captures并不能满足
    							//我们的要求，我们需要用自己handler来处理。
